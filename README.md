...
> 🔧 **Production-ready agent monitor for AI infrastructure. Part of the [Agent Infrastructure](https://github.com/yksanjo/agent-infrastructure) ecosystem.**
...
```bash
npm install @agent-infra/monitor
```
...
```typescript
import { Monitor } from '@agent-infra/monitor';

const monitor = new Monitor();
monitor.track('cpu', value);
monitor.alert('cpu > 90%', handler);
```
...
```typescript
const instance = new AgentMonitor(config?: Config);
```
...
const instance = new AgentMonitor(config);
```
...
```bash
git clone https://github.com/yksanjo/agent-monitor.git
cd agent-monitor
npm install
```
